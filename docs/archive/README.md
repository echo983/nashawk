# Nashawk Usenet Archive

These documents are historical design, implementation, and field-test notes for
the Nashawk Usenet backend work that has been merged into `main`.

Use [Usenet Piece Backend README](../Usenet-Piece-Backend-README.md) for current
operator-facing behavior, configuration, and troubleshooting.

Archived documents:

- [Usenet Piece Backend Design](Usenet-Piece-Backend.md)
- [Usenet Piece Backend Implementation Plan](Usenet-Piece-Backend-Plan.md)
- [Usenet Local Piece Eviction Plan](Usenet-Local-Piece-Eviction-Plan.md)
- [Usenet Nyuu Batch Upload Plan](Usenet-Nyuu-Batch-Upload-Plan.md)
- [Usenet Web UI Plan](Usenet-Web-UI-Plan.md)
- [Usenet Piece Discovery Plan](Usenet-Piece-Discovery-Plan.md)
- [Usenet Backend Test Status - 2026-07-11](Usenet-Backend-Test-Status-2026-07-11.md)
